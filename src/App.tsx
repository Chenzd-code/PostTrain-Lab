...
function ChapterOrientation({ chapter }: { chapter: Chapter }) {
  ...
  return <section className="chapter-orientation">
    ...
    {(guide.prerequisites || guide.readingAdvice) && <div className="orientation-notes">
      {guide.prerequisites && <div><strong>开始前先确认</strong><ul>{guide.prerequisites.map(item=><li key={item}>{item}</li>)}</ul></div>}
      {guide.readingAdvice && <div><strong>推荐学习方式</strong><p>{guide.readingAdvice}</p></div>}
    </div>}
